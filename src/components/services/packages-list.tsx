...
import { CheckCircle2, ShoppingCart, CreditCard, Landmark, Phone, Mail } from "lucide-react";
...
export default function PackagesList() {
    ...
    const packages = packagesData.map(pkg => ({
        ...pkg,
        image: PlaceHolderImages.find(img => img.id === pkg.id)
      }));
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {packages.map((pkg, index) => (
                    <div key={pkg.title} className="opacity-0 animate-fade-in-up" style={{ animationDelay: `${100 * (index + 1)}ms` }}>
                        <Card className="h-full flex flex-col bg-card/60 backdrop-blur-sm border-border/50 hover:border-primary transition-colors duration-300 overflow-hidden group">
                           {pkg.image && (
